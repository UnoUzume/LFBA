...
def get_labeled_data(data_dir, selected_label, n_samples, dtype='Train'):
	...
	for file in os.listdir(os.path.join(data_dir, features_path)):
		if file.startswith('_'.join([dtype, 'Normalized'])):
			print(f'Loading {os.path.join(data_dir, features_path, file)}.')
			df = pd.read_csv(
				os.path.join(data_dir, features_path, file), header=None, sep=' ', engine='c'
			)
	...
	if n_samples is None:
		return (
			data_X_image_selected.values[:],
			data_X_text_selected.values[:],
			np.argmax(selected.values[:], 1),
		)
	return (
		data_X_image_selected.values[:n_samples],
		data_X_text_selected.values[:n_samples],
		np.argmax(selected.values[:n_samples]),
	)
...
def split_vfl(data, args):
	...
	if args.dataset == 'PHISHING':
		...
		x_b = data[:, 15:]
		return x_a, x_b
	if args.dataset == 'NUSWIDET':
		# 634/1000
		x_a = data[:, :634]
		x_b = data[:, 634:]
		return x_a, x_b
	if args.dataset == 'NUSWIDEI':
		# 1000/634
		x_a = data[:, 634:]
		x_b = data[:, :634]
		return x_a, x_b
