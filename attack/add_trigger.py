...
def add_trigger_to_data(
	args, logger, poison_indexes, new_data, trigger_dimensions, new_targets, rate, mode, replace_label
):
	mode_print(logger, mode)
	if args.dataset == 'CIFAR10':
		new_data, new_targets = add_triangle_pattern_trigger(
			args, logger, poison_indexes, new_data, new_targets, rate, mode, replace_label
		)
...
def add_vector_replacement_trigger(
	args, logger, poison_indexes, trigger_dimensions, new_data, new_targets, rate, mode, replace_label
):
	for idx in poison_indexes:
		if replace_label and mode == 'train':
			new_targets[idx] = args.target_label
